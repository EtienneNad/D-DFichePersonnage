...
useEffect(() => {
  if (personnages.length > 0) {
    const chargerInfos = async () => {
      const nomR = await recupererRace(personnages[0].race_id ?? 0);
      const nomC = await recupererClasse(personnages[0].classe_id ?? 0);
      setNomRace(nomR);
      setNomClasse(nomC);
    };
    chargerInfos();
...
return (
  ...
  <View style={styles.personnageContainer}>
    ...
    <Text style={styles.personnageTexte}>Classe : {nomClasse}</Text>
    <Text style={styles.personnageTexte}>Race : {nomRace}</Text>
