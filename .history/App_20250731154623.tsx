...
function App(): JSX.Element {
  ...
  const [selectedRace, setSelectedRace] = useState<{ id: number; name: string } | null>(null);
  const [selectedClasse, setSelectedClasse] = useState<{ id: number; name: string } | null>(null);
  ...
  label: item.race,
  value: {id: item.id, name: item.race}
}));
const classeDropdownData = classe.map((item) => ({
  label: item.classe,
  value: {id: item.id, name: item.classe}
}));
...
<View style={styles.dropdownContainer}>
  <Dropdown
    style={[
      styles.dropdown,
      themeActuel === 'sombre' && { backgroundColor: 'black', borderColor: '#666' },
    ]}
    containerStyle={{ backgroundColor: themeActuel === 'sombre' ? 'black' : 'black' }}
  ...
  onChange={(item) => {
    setSelectedRace({ id: item.id, name: item.label });
  }}
/>
</View>

{selectedRace ? (
  <Text style={styles.selectedRace}>Race choisie : { selectedRace.name} id: {selectedRace.id}</Text>
) : null}
...
<View style={styles.dropdownContainer}>
  ...
  onChange={(item) => {
    setSelectedClasse({ id: item.id, name: item.label });
  }}
  />
</View>

{selectedClasse ? (
  <Text style={styles.selectedRace}>Classe choisie : { selectedClasse.name} id: {selectedClasse.id}</Text>
) : null}
