...
function App(): JSX.Element {
  const [nomRace, setNomRace] = useState<string>('');
const [nomClasse, setNomClasse] = useState<string>('');
...
useEffect(() => {
  if (personnages.length > 0) {
    const chargerInfos = async () => {
      const nomR = await recupererRace(personnages[0].race_id);
      const nomC = await recupererClasse(personnages[0].classe_id);
      setNomRace(nomR);
      setNomClasse(nomC);
    };
    chargerInfos();
...
return (
  ...
  <View style={styles.personnageContainer}>
    <Text style={styles.sectionTitle}>Fiche du personnage</Text>
    <Text style={styles.personnageTexte}>Nom : {personnages[0].nomPersonnage}</Text>
    <Text style={styles.personnageTexte}>Classe : {recupererClasse(personnages[0].classe_id)}</Text>
    <Text style={styles.personnageTexte}>Race : {recupererRace(personnages[0].race_id)}</Text>
    <Text style={styles.personnageTexte}>Âge : {personnages[0].age} ans</Text>
    <Text style={styles.personnageTexte}>Sexe : {personnages[0].sexe}</Text>
    <Text style={styles.personnageTexte}>Taille : {personnages[0].taille}</Text>
    <Text style={styles.personnageTexte}>Poids : {personnages[0].poids}</Text>
    <Text style={styles.personnageTexte}>Niveau : {personnages[0].niveau}</Text>
    <Text style={styles.personnageTexte}>Alignement : {personnages[0].alignement}</Text>
    <Text style={styles.personnageTexte}>PV : {personnages[0].pvActuel} / {personnages[0].pvMax}</Text>
    <Text style={styles.personnageTexte}>Attaque : {personnages[0].attaque}</Text>
    <Text style={styles.personnageTexte}>Sorts : {personnages[0].sort}</Text>
    <Text style={styles.personnageTexte}>Équipement : {personnages[0].equipement}</Text>
    <Text style={styles.personnageTexte}>Apparence : {personnages[0].apparence}</Text>
    <Text style={styles.personnageTexte}>Histoire : {personnages[0].histoire}</Text>
  </View>
)}
  </ScrollView>
);
